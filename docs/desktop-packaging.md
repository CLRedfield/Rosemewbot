...
- 直接下载的安装资源会在停止机器人前准备完成，减少 QQ 与机器人不可用的时间。
- 升级前只保存本次会变化的 AstrBot 数据、NapCat、uv 工具环境和组件清单；Windows 使用多线程文件复制，仅保留最近一份可回滚快照。
- 安装后验证组件版本、启动文件、AstrBot 平台配置和 NapCat WebSocket 客户端；原本健康运行的服务还会接受启动后探测。
- 任一阶段失败都会显示准确阶段；已经修改组件时自动恢复升级前快照，并验证旧服务是否恢复。用户也可以在“组件兼容中心”手动回滚。
...
- 停止进程前同时核对 PID 与可执行文件名；关联 QQ 只在加载了 `native-runtime` 内受管 Hook DLL 时才会被关闭，避免 Windows 重启或 PID 复用造成误杀。
